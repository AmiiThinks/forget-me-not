...
def test_keyed_parsable():
    ...
    #print(t)
    # so a keyed thing really has to be alone
...
"""
I don't think we're using Included any more
class TestIncludedKeyed(Structure):
    _fields = [Keyed('prime', required=True, keyword=False, positional=True, included=True),
               Keyed('reqKey', required=True, keyword=True, positional=False, included=True),
               Keyed('optSilent', required=False, keyword=False, positional=False, included=True),
               Keyed('optKey', required=False, keyword=True, positional=False, included=True),
               ]


@unittest.skip
def test_keyed_from_dict():
    t = TestIncludedKeyed(reqKey={'b': 'rk'}, optKey={'d': 'ok'}, prime={'a': 'pr'}, optSilent={'c': 'os'})
    print(t.get_arg_string())
    print(t.get_dir_string())
    
@unittest.skip
def test_keyed_from_strings():
    t = TestIncludedKeyed(reqKey='b-rk', optKey='d-ok', prime='a-pr', optSilent='d-os')
    assert t.get_arg_string() == '--prime a-pr --reqKey b-rk --optSilent c-os --optKey d-ok'
    assert t.get_dir_string() == 'a-pr/c-os_optKey-d-ok_reqKey-b-rk'
    assert t.optSilent == {'c': 'os'}

@unittest.skip   
def test_keyed_from_list():
    t = TestIncludedKeyed(reqKey=['a-0', 'b-1', 'c-2'], prime=['pr-0-1-2', 'a-0-1'])
    assert t.get_arg_string() == '--prime pr-0-1-2 a-0-1 --reqKey a-0 b-1 c-2'
    assert t.get_dir_string() == ''
